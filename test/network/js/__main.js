...
function configSigmaElements(config) {
    ...
    $GP.bg = $(sigInst.renderers[0].domElements.bg);
    $GP.bg2 = $(sigInst.renderers[0].domElements.bg2);
    var a = [],
        b,x=1;
    ...
    $("#zoom").find("div.z").each(function () {
        ...
        a.click(function () {
			if (b == "center") {
				sigInst.position(0,0,1).refresh();
			} else {
		        var a = sigInst._core;
...
function nodeNormal() {
    !0 != $GP.calculating && !1 != sigInst.detail && (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(400).animate({width:'hide'},350),$GP.cluster.hide(), sigInst.graph.nodes().forEach(function (a) {
        a.color = !1;
        a.hidden = !1
    }), sigInst.graph.nodes().forEach(function (a) {
        a.hidden = !1;
        ...
        a.lineWidth = !1;
        a.size = !1
    }), sigInst.refresh(2, 2, 2, 2), sigInst.neighbors = {}, sigInst.active = !1, $GP.calculating = !1, window.location.hash = "")
}
...
function nodeActive(a) {
    ...
    sigInst.neighbors = {};
    sigInst.detail = !0;
    var b = sigInst.renderers[0].graph.nodesIndex[a];
    showGroups(!1);
	var outgoing={},incoming={},mutual={};//SAH
    sigInst.graph.nodes().forEach(function (b) {
        b.attr.lineWidth = !1;
        b.hidden = !0;
    ...
    var createList=function(c) {
    	...
    	var e = [],
       		 ...
       		 g;
    for (g in c) {
        var d = sigInst.renderers[0].graph.nodesIndex[g];
        d.hidden = !1;
        d.attr.lineWidth = !1;
    ...
    d = "";
		for (g in e) {
			...
			f.push('<li class="membership"><a href="#' + c.name + '" onmouseover="sigInst.renderers[0].plotter.drawHoverNode(sigInst.renderers[0].graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + c.name + "</a></li>");
		}
		return f;
	}
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
        	...
        	$GP.info_name.html("<div><img src=" + f.attributes[image_attribute] + " style=\"vertical-align:middle\" /> <span onmouseover=\"sigInst.renderers[0].plotter.drawHoverNode(sigInst.renderers[0].graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + b.label + "</span></div>");
        } else {
        	$GP.info_name.html("<div><span onmouseover=\"sigInst.renderers[0].plotter.drawHoverNode(sigInst.renderers[0].graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + b.label + "</span></div>");
        }
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
            ...
            !0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<li class="membership"><a href="#'+
			d.label+
			'" onmouseover="sigInst.renderers[0].dispatchEvent(\'overNode\', {node:sigInst.graph.nodes(\''+
			d.id+
			'\')})\" onclick=\"nodeActive(\'" + d.id + \')" onmouseout="sigInst.refresh()">' + d.label + "</a></li>"))
        }
